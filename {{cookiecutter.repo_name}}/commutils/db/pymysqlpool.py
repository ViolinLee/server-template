...
site: https://github.com/jkklee/pymysql-pool/tree/master
"""
import pymysql
import warnings
import logging
            ...
            if retry_num > 0:
                ...
                if self.total_num < self.maxsize:
                    ...
                    raise GetConnectionFromPoolError(
                        "can't get connection from pool({}), due to pool lack.".format(self.name))
...
def already_returned_conn(f):
    ...
    def wrapper(*args, **kwargs):
        ...
        if hasattr(args[0], '_returned') and args[0]._returned:
            raise ReturnConnectionToPoolError(
                "this connection has already returned to the pool({})".format(args[0]._pool.name))
